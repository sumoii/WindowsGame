...
HBITMAP	g_hSprite[4] , g_hBackGround = NULL;//定义三个位图句柄，用于3张图片存放
DWORD		g_tPre = 0, g_tNow = 0;					//声明l两个函数来记录时间,g_tPre记录上一次绘图的时间，g_tNow记录此次准备绘图的时间
int g_iPicNum = 0, g_iX = 0, g_iY = 0;//"g_iNum"变量用来记录目前显示的图号
Sprites Sprite[SPRITE_NUMBER]; //按照SPRITE_NUMBER 的值建立数组Sprite[]
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	PlaySound(L"../../../../resource/Chapter6/GDIdem9/OrcTheme.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP); //循环播放背景音乐 
...
BOOL Game_Init(HWND hwnd)
{
	...
	//设定初始的贴图坐标都为窗口内的任意坐标，初始的移动方向为向左。
...
{
	//处理图号
	if (g_iPicNum == 4)               //判断是否超过最大图号，若超过最大图号“12”，则将显示图号重设为"0"。
		g_iPicNum = 0;
	...
	g_iPicNum++;                    //将“g_iNum”值加1，为下一次要显示的图号
