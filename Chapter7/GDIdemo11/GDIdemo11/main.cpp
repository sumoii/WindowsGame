...
HBITMAP	g_hSwordMan = NULL , g_hSwordBlade = NULL, g_hBackGround = NULL;//定义三个位图句柄，用于3张图片存放
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd)
{
	...
	PlaySound(L"../../../../resource/Chapter7/GDIdem11/仙剑三·原版战斗3.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP); //循环播放背景音乐 
	...
	if(g_iBulletNum!=0)
		for (int i = 0; i < 30; i++)
			if (Bullet[i].exist)
			{
				...
				//设置下一个剑气（子弹）的坐标。剑气（子弹）是从右向左发射的，因此，每次其X轴上的坐标值递减10个单位，这样贴图会产生往左移动的效果。
				//而如果剑气（子弹）下次的坐标已超出窗口的可见范围（h[i].x<0）,
				//那么剑气（子弹）设为不存在，并将剑气（子弹）总数g_iBulletNum变量值减1.
				Bullet[i].x -= 10;
				if (Bullet[i].x < 0)
				{
					g_iBulletNum--;
